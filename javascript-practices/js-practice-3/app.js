...
menuContainer = document.querySelector(".section-center");

let filters = ["All", "Korea", "Japan", "China"];

filters.forEach((filter) => {
    let filterBtn = document.createElement("button");
    filterBtn.classList.add("btn");
    filterBtn.classList.add("btn-outline-secondary");
    filterBtn.classList.add("m-2");
...
function filterMenu(event) {
    ...
    if (event.target.textContent !== "All") {
        filteredMenu = menu.filter(item => item.category === event.target.textContent);
    }
...
function updateMenu(menu) {
    ...
    let newItem
    menu.forEach((meal) => {
        newItem = `<div class="menu-items col-lg-6 col-sm-12">
